...
// buttons
document.getElementById("normalButton").addEventListener("click", normalGame);
...
var nhscore = localStorage.getItem("normalhighscore");
let score = 0;
...
function draw(){
    ...
    if(snakeX < box || snakeX > 17 * box || snakeY < 3*box || snakeY > 17*box || collision(newHead,snake)){
        ...
        if (score>nhscore){
            nhscore = score;
            localStorage.setItem("normalhighscore", nhscore);
    ...
    ctx.fillText("Highscore: "+ nhscore,12.5*box,1.2*box);
...
function normalGame(){
    ...
    game = setInterval(draw,125);//normal
    snake[0] = {
        x : 9 * box,
        y : 10 * box
    };
