...
// buttons
document.getElementById("easyButton").addEventListener("click", easyGame);
...
function draw(){
    ...
    if(snakeX < box || snakeX > 17 * box || snakeY < 3*box || snakeY > 17*box || collision(newHead,snake)){
        ...
        if (score>ehscore){
            ehscore = score;
            localStorage.setItem("easyhighscore", ehscore);
    ...
    ctx.fillText("Highscore: "+ ehscore,12.5*box,1.2*box);
...
function easyGame(){
    ...
    game = setInterval(draw,200);//easy
    snake[0] = {
        x : 9 * box,
        y : 10 * box
    };
